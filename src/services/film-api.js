...
export function fetchFilmsByKeyWord(name) {
  return fetchWithErrorHandling(`${BASE_URL}/search/movie?api_key=${KEY}&query=${name}&page=1&include_adult=false`);
}
...
export function fetchCastForFilm(movie_id) {
  return fetchWithErrorHandling(`${BASE_URL}/movie/${movie_id}/credits?api_key=${KEY}`);
}
export function fetchReviewsForFilm(movie_id) {
  return fetchWithErrorHandling(`${BASE_URL}/movie/${movie_id}/reviews?api_key=${KEY}`);
}